...
public class SceneSwitcher : Singleton<SceneSwitcher> {
    ...
    public static IEnumerator LoadOuterScene(string sceneName, SceneTransitionType sceneTransitionType, string entryName) {
        if (SceneUtility.GetBuildIndexByScenePath(sceneName) < 0) yield break;
        if (!string.IsNullOrEmpty(CurrentOuterScene) && CurrentOuterScene != sceneName) {
            yield return UnloadOuterScene();
        }
